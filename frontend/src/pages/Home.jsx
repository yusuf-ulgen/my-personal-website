    ...
    <div id="top" className="min-h-screen bg-[#0B0616] text-[#E2DCE7] selection:bg-[#d946ef] selection:text-white font-sans relative overflow-x-hidden">
      ...
      <footer className="relative z-10 max-w-7xl mx-auto px-6 py-12 flex flex-col items-center text-center">
        <div className="flex flex-wrap justify-center gap-10 md:gap-16 w-full max-w-2xl text-[#8c8496] text-sm font-medium tracking-wide">
          <a href={formatExternalLink(profile?.githubLink)} target="_blank" rel="noreferrer" className="hover:text-white transition-colors">GitHub ↗</a>
          <a href={formatExternalLink(profile?.linkedinLink)} target="_blank" rel="noreferrer" className="hover:text-white transition-colors">LinkedIn ↗</a>
          <a href={formatMailtoLink(profile?.email)} className="hover:text-white transition-colors">E-Posta ↗</a>
        </div>
          ...
          <div className="relative w-full max-w-5xl max-h-[90vh] overflow-y-auto bg-[#0A0510] border border-white/10 rounded-3xl shadow-2xl p-8 md:p-16 animate-[fadeInUp_0.4s_ease-out]">
            ...
            <div className="flex flex-wrap gap-4">
              {selectedProject.webUrl && (
                <a href={formatExternalLink(selectedProject.webUrl)} target="_blank" rel="noreferrer" className="bg-white text-black px-8 py-3 rounded-full font-bold transition-transform hover:scale-105 flex items-center gap-2">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9"></path></svg>
                  {t.modal.webSite}
                </a>
              )}
              {selectedProject.mobileUrl && (
                <a href={formatExternalLink(selectedProject.mobileUrl)} target="_blank" rel="noreferrer" className="bg-white text-black px-8 py-3 rounded-full font-bold transition-transform hover:scale-105 flex items-center gap-2">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z"></path></svg>
                  {t.modal.mobileApp}
                </a>
              )}
              {selectedProject.liveUrl && !selectedProject.webUrl && (
                <a href={formatExternalLink(selectedProject.liveUrl)} target="_blank" rel="noreferrer" className="bg-white text-black px-8 py-3 rounded-full font-bold transition-transform hover:scale-105 flex items-center gap-2">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path></svg>
                  {t.modal.liveSite}
                </a>
              )}
              {selectedProject.githubUrl && (
                <a href={formatExternalLink(selectedProject.githubUrl)} target="_blank" rel="noreferrer" className="bg-white/5 border border-white/10 text-white px-8 py-3 rounded-full font-bold transition-transform hover:scale-105 flex items-center gap-2">
